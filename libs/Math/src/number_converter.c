...
int str_to_int32(const char * input_p, int32_t * output_p)
{
    ...
    if ((0 != errno) && (0 == result))
    {
        PRINT_DEBUG("strtol() unknown error.");
        goto END;
    }
    ...
    {
        PRINT_DEBUG("signed 32-bit overflow detected.");
        goto END;
    }
    ...
    if (INT32_MAX < result)
    {
        PRINT_DEBUG("signed 32-bit integer overflow detected.");
        goto END;
    }
    ...
    if (INT32_MIN > result)
    {
        PRINT_DEBUG("signed 32-bit integer underflow detected.");
        goto END;
    }
    ...
    if (endptr == input_p)
    {
        PRINT_DEBUG("strtol() invalid input - no digits found.");
        goto END;
    }
    ...
    if ('\0' != *endptr)
    {
        PRINT_DEBUG("strtol() invalid input - extra characters.");
        goto END;
    }
...
int str_to_uint32(const char * input_p, uint32_t * output_p)
{
    ...
    if ((0 != errno) && (0 == result))
    {
        PRINT_DEBUG("strtoul() unknown error.");
        goto END;
    }
    ...
    {
        PRINT_DEBUG("unsigned long overflow detected.");
        goto END;
    }
    ...
    {
        PRINT_DEBUG("unsigned 32-bit integer overflow detected.");
        goto END;
    }
    ...
    if (endptr == input_p)
    {
        PRINT_DEBUG("strtoul() invalid input - no digits found.");
        goto END;
    }
    ...
    if ('\0' != *endptr)
    {
        PRINT_DEBUG("strtoul() invalid input - extra characters.");
        goto END;
    }
...
int str_to_uint16(const char * input_p, uint16_t * output_p)
{
    ...
    {
        PRINT_DEBUG("unsigned 16-bit integer overflow detected.");
        goto END;
    }
...
int uint32_to_str(uint32_t input, char ** output_p)
{
    ...
    if (NULL == output_p)
    {
        PRINT_DEBUG("uint32_to_str(): NULL argument passed.");
        goto END;
    }
    ...
    *output_p = calloc(UINT32_LEN, sizeof(char));
    if (NULL == *output_p)
    {
        PRINT_DEBUG("uint32_to_str(): CMR failure.");
        goto END;
    }
    ...
    if (0 > result)
    {
        PRINT_DEBUG("uint32_to_str(): snprintf() failure.");
        free(*output_p);
        *output_p = NULL;
        goto END;
    }
