                  ...
                  int                fd_capacity)
{
    ...
    if (NULL == sock_mgr)
    {
        PRINT_DEBUG("sock_mgr_init(): NULL argument passed.\n");
        goto END;
    }

    if (0 >= max_fds)
    {
        PRINT_DEBUG("sock_fd_arr_init(): 'max_fds' must be greater than 0\n");
        goto END;
    }
    ...
    {
        PRINT_DEBUG("sock_mgr_init(): Unable to setup fd array.\n");
        goto END;
    }
...
int sock_fd_add(socket_manager_t * sock_mgr, int new_fd)
{
    ...
    if ((NULL == sock_mgr))
    {
        PRINT_DEBUG("sock_fd_add(): NULL argument passed.\n");
        goto END;
    }
    ...
    if (sock_mgr->fd_count == sock_mgr->fd_capacity)
    {
        ...
        if (E_SUCCESS != exit_code)
        {
            PRINT_DEBUG("sock_fd_add(): Unable to check capacity.\n");
            goto END;
        }
    }
...
int sock_fd_remove(socket_manager_t * sock_mgr, int index)
{
    ...
    if (NULL == sock_mgr)
    {
        PRINT_DEBUG("sock_fd_remove(): NULL argument passed.\n");
        goto END;
    }
...
int sock_fd_increase_capacity(socket_manager_t * sock_mgr)
{
    ...
    if (NULL == sock_mgr)
    {
        PRINT_DEBUG("sock_fd_increase_capacity(): NULL argument passed.\n");
        goto END;
    }
    ...
    if (NULL == new_array)
    {
        PRINT_DEBUG("sock_fd_increase_capacity(): CMR failure - 'fd_arr'.\n");
        goto END;
    }
...
int sock_fd_arr_init(socket_manager_t * sock_mgr, int server_fd)
{
    ...
    if (NULL == sock_mgr)
    {
        PRINT_DEBUG("sock_fd_arr_init(): NULL argument passed.\n");
        goto END;
    }
    ...
    {
        PRINT_DEBUG("sock_fd_arr_init(): CMR failure - 'fd_arr'.\n");
        goto END;
    }
...
void print_fd_array(socket_manager_t * sock_mgr)
{
    if (sock_mgr == NULL)
    {
        PRINT_DEBUG("print_fd_array(): NULL socket manager.\n");
        goto END;
    }
