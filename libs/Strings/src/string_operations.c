...
int strip_newline(char * input_str_p)
{
    ...
    if (NULL == input_str_p)
    {
        PRINT_DEBUG("strip_newline(): NULL argument passed.");
        goto END;
    }
    size_t len         = 0;
...
int strings_are_equal(const char * str1_p, const char * str2_p, int * result_p)
{
    ...
    {
        PRINT_DEBUG("strings_are_equal(): NULL argument passed.");
        goto END;
    }
                ...
                bool         allow_truncation)
{
    ...
    {
        PRINT_DEBUG("copy_string(): NULL argument passed.");
        goto END;
    }
    ...
    if (NULL == buffer)
    {
        PRINT_DEBUG("copy_string(): buffer - CMR failure.");
        goto END;
    }
    ...
    if (num_characters >= buffer_size)
    {
        ...
        {
            PRINT_DEBUG("copy_string(): String truncation detected.");
            free(buffer);
            buffer = NULL;
            goto END;
        }
