...
static void graph_cleanup(matrix_t ** graph)
{
    ...
    {
        PRINT_DEBUG("graph_cleanup(): NULL argument passed.");
        goto END;
    }
                       ...
                       FREE_F custom_free)
{
    ...
    {
        PRINT_DEBUG("matrix_init(): NULL argument passed.");
        goto END;
    }
    ...
    if (NULL == graph)
    {
        PRINT_DEBUG("matrix_init(): graph - CMR failure.");
        graph_cleanup(&graph);
        goto END;
    }
    ...
    if (NULL == graph->edges)
    {
        PRINT_DEBUG("matrix_init(): graph->edges - CMR failure.");
        graph_cleanup(&graph);
        goto END;
    }
    ...
    for (size_t idx = 0; idx < num_nodes; idx++)
    {
        ...
        if (NULL == graph->edges[idx])
        {
            PRINT_DEBUG("matrix_init(): graph->edges[idx] - CMR failure.");
            graph_cleanup(&graph);
            goto END;
        }
    }
    ...
    if (NULL == graph->nodes)
    {
        PRINT_DEBUG("matrix_init(): graph->nodes - CMR failure.");
        graph_cleanup(&graph);
        goto END;
    }
                         ...
                         const char * label)
{
    ...
    {
        PRINT_DEBUG("matrix_populate_node(): NULL argument passed.");
        goto END;
    }
    ...
    // {
    //     PRINT_DEBUG("matrix_populate_node(): Unable to copy label.");
    //     goto END;
    // }
...
int matrix_depopulate_node(matrix_t * graph, size_t index)
{
    ...
    if (NULL == graph)
    {
        PRINT_DEBUG("matrix_depopulate_node(): NULL argument passed.");
        goto END;
    }
